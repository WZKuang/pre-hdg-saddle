...
def SolveProblem(order=1, refines=3, dim=3, tau=0, ilam=1, hd=True, red=True, 
    ...
    for i in range(refines):
        ...
        N = BilinearForm(W0)
        N += 1/tau/h*(p0-p0.Other())*(q0-q0.Other())*dx(skeleton=True)
        N += ilam*p0*q0*dx
        ...
        with TaskManager():
            ...
            jac = a.mat.CreateSmoother(udofs)
            ...
            tmp.FV().NumPy()[:] = random.rand(fes.ndof)/100
...
for dim in dimList:
    for order in orderList:
        for ilam in ilamList:
            SolveProblem(order, refines, dim=dim, tau=0, ilam=ilam, hd=True, red=False,
                    pr=False, draw=False)
